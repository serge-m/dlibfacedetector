...
class DlibFaceExtractor(SubimageExtractor):
    def __init__(self, path_face_detector_model, imgDim: int):
        self.dst_image_size = imgDim
        self._face_detector = DlibFaceDetector()
        self._predictor = dlib.shape_predictor(path_face_detector_model)
    ...
    def extract(self, img_rgb: np.ndarray) -> List[SubImage]:
        ...
        for face_rectange in rectangles:
            points = self._predictor(img_rgb, face_rectange)
            landmarks = list(map(lambda p: (p.x, p.y), points.parts()))
            npLandmarks = np.float32(landmarks)
            npLandmarkIndices = np.array(landmarkIndices)
            H = cv2.getAffineTransform(npLandmarks[npLandmarkIndices],
                                       self.dst_image_size * MINMAX_TEMPLATE[npLandmarkIndices])
            thumbnail = cv2.warpAffine(img_rgb, H, (self.dst_image_size, self.dst_image_size))
